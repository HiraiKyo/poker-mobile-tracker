      ...
      <ScrollView>
        <View style={{ alignItems: "center", flex: 1}}>
          ...
          <Picker
            ...
            onValueChange={(itemValue, itemIndex) => setSelectedChart(itemValue)}
            style={{ color: Colors[colorScheme ?? "light"].text }}
          >
            <Picker.Item label="ALL" value="all" />
            <Picker.Item label="KKPoker $100-$500 Flash" value="kkpoker-100-500-flash" />
          </Picker>
          <ChartArea />
  ...
  return (
    ...
    >
          ...
          {summaryData.labels.map( (label, index) => (
            <View style={{ flex: 1 / summaryData.labels.length }} key={`label_${index}`}>
              <Text>{label}</Text>
            </View>))}
          </View>
          {summaryData.datasets.map((dataset, index) => (
            <View style={styles.tableRow} key={`dataset_${index}`}>
              {dataset.data.map((value, index) => (
                <View style={{ flex: 1 / summaryData.labels.length }} key={`data_${index}`}>
                  <Text>{value}</Text>
                </View>
              ))}
            </View>
...
const data = {
  labels: ['12/1', '12/2', '12/3', '12/4', '12/5', '12/6', '12/7', '12/8', '12/9', '12/10', '12/11', '12/12', '12/13', '12/14', '12/15', '12/16'],
  datasets: [{
    data: [
      50,
      20,
...
const summaryData = {
  ...
  datasets: [{
    data: [
      "総計", 125012, 4250, 4250/125012 * 100
    ]
  }, {
    data: [
      "総計", 125012, 4250, 4250/125012 * 100
